...
Regional Platform resources should be deployed to us-east1 unless an Architecture Decision Record (ADR) explicitly approves another region.

Global Google Cloud services, such as IAM, Secret Manager, Cloud Logging, and Cloud Monitoring, remain global and do not require regional selection.

Maintaining a single primary deployment region simplifies operations, reduces latency variability, and provides a consistent deployment model.
